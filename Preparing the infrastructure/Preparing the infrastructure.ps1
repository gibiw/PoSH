...
# Функция для создания графических элементов
...
function createOU {
        ...
        [string]$PathCreateGroups=$settingsOU.PathCreateGroups
        # Путь где будет создана OU с серверами
        [string]$PathCreateServers=$settingsOU.PathCreateServers
        # Путь где будет создана ресурсная группа
        [string]$PathCreateGroup=$settingsOU.PathCreateGroup
        # Имя создаваемой группы (название ИС + NameGroupk)
        [string]$NameGroupk=$settingsOU.NameGroupk
        # Названия групп, которые будут по умолчанию включеный в созданную группу
        [string]$DefaultAddMember=$settingsOU.DefaultAddMember
        ...
        Start-Sleep -Seconds 1
        try{
            Get-ADGroup $groupname -ErrorAction Stop | Add-ADGroupMember -Members $Usergroupname -ErrorAction Stop    
            }#end try
        catch{
            LogAdd ("Ошибка при добавлении группы $Usergroupname в группу $groupname" + $_)
            }#end catch
        if ($DefaultAddMember){
...
function createCMP {
        ...
        [string]$DNSZone=$settingsCMP.DNSZone
        # Сервер, который хостит необходимую ДНС зону (DNSZone)
        [string]$DNSServer=$settingsCMP.DNSServer
        # Группа рассылки, на которую будут отправлены файлы ключей
        [string]$KeyRecipient=$settingsCMP.KeyRecipient
        ...
        if ($CheckBox1.Checked -eq $true) {
                    ...
                    $Attachments = "$folderPathkeys\$folder.7z"
...
function createuser {
        ...
        [string]$OUuser=$settingsUser.OUuser
        # Текст, который выдается в форме при дублирующем пользователей
        [string]$TextForm=$settingsUser.TextForm
        # Содержание тела письма СУД
        [string]$BodySud=$settingsUser.BodySud
        # Адрес потчового ящмка СУД 
        [string]$SudAddress=$settingsUser.SudAddress
        # Содержание отправляемого файла
        [string]$fileuser=$settingsUser.filetext
        # Содержание тела письма (ОЕМ)
        [string]$body1=$settingsUser.Body1
        # Содержание тела письма (доступ админов)
        [string]$body2=$settingsUser.Body2        
        # Содержание тела письма (Система контроля версий)
        [string]$body3=$settingsUser.Body3        
        # Тема письма (ОЕМ)
        [string]$Subject1=$settingsUser.Subject1        
        # Тема письма (доступ админов)
        [string]$Subject2=$settingsUser.Subject2        
        # Тема письма (Система контроля версий)
        [string]$Subject3=$settingsUser.Subject3
        ...
        foreach ($User in $Users){
            ...
            if ($tempuser){
                    ...
                    create_label ($operror1 = New-Object windows.Forms.Label) $texttemp 30 200 240 15 $Form1 a
                    ...
                    # кнопка отмена
                    create_label ($buttonoff = New-Object system.Windows.Forms.Button ) 'Отмена' 220 285 100 30 $Form1
                    $buttonoff.Add_Click($Function:off) 
                    # кнопка создания
                    create_label ($buttoncrt = New-Object system.Windows.Forms.Button ) 'Создать' 50 285 100 30 $Form1  
            ...
            if($sam -and $Detailedname){
                    ...
                    try {
                        ...
                        -AccountPassword (ConvertTo-SecureString $Password -AsPlainText -Force) -Enabled $true -Path $OUuser -EmailAddress $EmailAddress -Description $Description `
                    ...
                    if ($sendsyd -eq $true){
                        $Encoding = [System.Text.Encoding]::UTF8
                        $body=$BodySud -replace "Sam",$sam
                        $body=$Body -replace  "Detailedname",$Detailedname
                        $body=$Body -replace  "Password",$Password
                        try {
                            if($SMTPServerExternalSSL -eq $true){
                                send-mailmessage -SmtpServer $SMTPServerExternal -From $from -Subject 'Задания на изменение' -To $SudAddress -Body $body -Credential $mycreds -DeliveryNotificationOption OnSuccess -Port $SMTPServerExternalPort -Encoding $Encoding -UseSsl -ErrorAction Stop
                                }#end if
                            else{
                                send-mailmessage -SmtpServer $SMTPServerExternal -From $from -Subject 'Задания на изменение' -To $SudAddress -Body $body -Credential $mycreds -DeliveryNotificationOption OnSuccess -Port $SMTPServerExternalPort -Encoding $Encoding -ErrorAction Stop
                                }#end else
                            LogAdd ("Данные отправлены на адрес СУД")    
                            }#end try
                        catch [System.Exception] {
...
function resetpassword{
        ...
        [string]$Body=$settingsResetPassword.Body 
        # Тема письма (Сброс пароля)
        [string]$Subject=$settingsResetPassword.Subject
        # Папка, где будут лежать данные после сброса пароля
        [string]$Path=$settingsResetPassword.Path
